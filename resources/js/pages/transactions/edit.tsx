...
interface ExistingDetail {
    kode_produk: string; nama_produk: string; qty: number;
    harga: string; disc1: string; disc2: string; disc3: string; harga_net: string; jumlah: string;
}
interface Transaction { id: number; no_inv: string; kode_customer: string; tgl_inv: string; details: ExistingDetail[]; }
interface Props { transaction: Transaction; products: Product[]; customers: Customer[]; }
...
export default function TransactionEdit({ transaction, products, customers }: Props) {
    const initialDetails: DetailRow[] = transaction.details.map((d) => {
        const p = products.find((p) => p.kode_produk === d.kode_produk);
        return {
            kode_produk: d.kode_produk, nama_produk: d.nama_produk, qty: d.qty,
            harga: parseFloat(d.harga), disc1: parseFloat(d.disc1), disc2: parseFloat(d.disc2),
            disc3: parseFloat(d.disc3), harga_net: parseFloat(d.harga_net), jumlah: parseFloat(d.jumlah),
            stok_tersedia: (p?.stok ?? 0) + d.qty,
        };
    });

    const [details, setDetails] = useState<DetailRow[]>(initialDetails);
    const { data, setData, put, processing, errors } = useForm<{
        kode_customer: string; tgl_inv: string; details: Omit<DetailRow, 'stok_tersedia'>[];
    }>({ kode_customer: transaction.kode_customer, tgl_inv: transaction.tgl_inv, details: [] });

    useEffect(() => {
        setData('details', details.map(({ stok_tersedia: _, ...rest }) => rest));
    }, [details]);

    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); put(transactions.update(transaction.id).url); };
    ...
    const updateDetail = (i: number, field: keyof DetailRow, value: string | number) => {
        setDetails((prev) => {
            ...
            if (field === 'kode_produk') {
                ...
                if (p) {
                    const old = initialDetails.find((d) => d.kode_produk === value);
                    row.nama_produk = p.nama_produk; row.harga = parseFloat(p.harga);
                    row.stok_tersedia = p.stok + (old?.qty ?? 0);
                }
            }
    ...
    return (
        <>
            <Head title={`Edit ${transaction.no_inv}`} />
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <Card className="rounded-2xl border-slate-200 shadow-sm">
                        <CardHeader className="border-b border-slate-100 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <CardTitle className="text-xl text-slate-800">Edit Transaksi</CardTitle>
                                    <CardDescription className="font-mono text-sm text-slate-500">{transaction.no_inv}</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="grid gap-4 pt-5 md:grid-cols-3">
                            <div className="space-y-1.5">
                                ...
                                <Input value={transaction.no_inv} readOnly className="bg-slate-50 font-mono text-slate-500" />
                            </div>
                            <div className="space-y-1.5">
                    ...
                    <Card className="rounded-2xl border-slate-200 shadow-sm">
                        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 pb-4">
                            <div>
                                ...
                                <CardDescription className="text-xs text-slate-500">Edit produk yang dibeli.</CardDescription>
                            </div>
                            <Button type="button" size="sm" className="gap-1.5 bg-blue-600 hover:bg-blue-700"
                                onClick={() => setDetails((p) => [...p, { kode_produk: '', nama_produk: '', qty: 1, harga: 0, disc1: 0, disc2: 0, disc3: 0, harga_net: 0, jumlah: 0, stok_tersedia: 0 }])}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex gap-3">
                            <Button type="submit" disabled={processing} className="bg-amber-500 hover:bg-amber-600">Perbarui Transaksi</Button>
